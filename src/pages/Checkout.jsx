...
import { ChevronLeft, CreditCard, MapPin, Phone, User, Mail, CheckCircle2, Truck } from 'lucide-react';
  ...
  const placeOrder = async () => {
    ...
    try {
      const payload = {
        ...
        shippingAddress: { address: form.address, city: form.city, state: form.state, pincode: form.pincode, country: form.country },
        paymentMethod,
        subtotal: cartTotal,
    ...
    <div className="min-h-screen pt-20 pb-24 flex flex-col items-center justify-center px-4" style={{ background: '#F7F5F0' }}>
      <div className="bg-white rounded-3xl p-8 max-w-sm w-full text-center shadow-sm animate-fade-up">
        ...
        <div className="bg-[#F7F5F0] rounded-2xl p-4 mb-6 text-left space-y-2">
          <div className="flex justify-between text-sm"><span className="text-[#ABABAB] font-semibold">Total Paid</span><span className="font-black text-[#B8860B]">₹{total.toLocaleString()}</span></div>
          <div className="flex justify-between text-sm"><span className="text-[#ABABAB] font-semibold">Payment</span><span className="font-bold text-[#1C1C1C] capitalize">{paymentMethod}</span></div>
          <div className="flex justify-between text-sm"><span className="text-[#ABABAB] font-semibold">Delivery</span><span className="font-bold text-green-500">3-5 business days</span></div>
        </div>
    ...
    <div className="min-h-screen pb-32 lg:pb-12" style={{ background: '#F7F5F0' }}>
      <div className="max-w-4xl mx-auto px-4 sm:px-6 pt-20">
        ...
        <div className="flex items-center gap-3 py-5">
          ...
          <div>
            <h1 className="text-xl font-black text-[#1C1C1C]">{step === 1 ? 'Delivery Address' : 'Payment Method'}</h1>
        ...
        <div className="flex items-center gap-2 mb-6">
          {[1,2].map(s => (
            <React.Fragment key={s}>
              <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-black transition-all ${step >= s ? 'text-white' : 'bg-white text-[#ABABAB] border border-[#EBEBEB]'}`}
            ...
            {step === 1 && (
              <div className="bg-white rounded-3xl p-5 shadow-sm animate-fade-in space-y-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {[
                    { k:'name', label:'Full Name', icon:User, placeholder:'John Doe' },
                  ...
                  ].map(({ k, label, icon: Icon, placeholder, type='text' }) => (
                    <div key={k}>
                      ...
                      <div className="relative">
                        ...
                        <input type={type} value={form[k]} onChange={e => setF(k, e.target.value)}
                          placeholder={placeholder} className="inp pl-10 py-3 text-sm font-semibold" />
                      </div>
                    </div>
                  ))}
                </div>
                ...
                <div>
                  ...
                  <textarea value={form.address} onChange={e => setF('address', e.target.value)}
                    placeholder="House no., Street, Area…" rows={2} className="inp text-sm font-semibold resize-none" />
                </div>
                  ...
                  {[
                    ...
                    <div key={k}>
                      ...
                      <input value={form[k]} onChange={e => setF(k, e.target.value)}
                        placeholder={placeholder} className="inp py-3 text-sm font-semibold" />
                    </div>
                  ))}
                </div>
            ...
            {step === 2 && (
              <div className="space-y-3 animate-fade-in">
                  ...
                  <div className="space-y-2">
                    {PAYMENT_METHODS.map(pm => (
                      <button key={pm.id} onClick={() => setPaymentMethod(pm.id)}
                        className={`w-full flex items-center gap-3 p-4 rounded-2xl border-2 transition-all text-left ${paymentMethod === pm.id ? 'border-[#C9A227] bg-[#FFF8E1]' : 'border-[#EBEBEB] bg-white hover:border-[#C9A22750]'}`}>
                        <span className="text-2xl">{pm.icon}</span>
                        <span className={`font-bold text-sm ${paymentMethod === pm.id ? 'text-[#B8860B]' : 'text-[#1C1C1C]'}`}>{pm.label}</span>
                        <div className={`ml-auto w-5 h-5 rounded-full border-2 flex items-center justify-center ${paymentMethod === pm.id ? 'border-[#C9A227]' : 'border-[#EBEBEB]'}`}>
                          {paymentMethod === pm.id && <div className="w-2.5 h-2.5 rounded-full" style={{ background: '#C9A227' }} />}
                        </div>
                      </button>
                    ))}
                  </div>
                ...
                {paymentMethod === 'card' && (
                  <div className="bg-white rounded-3xl p-5 shadow-sm animate-fade-in space-y-3">
                    <div>
                      ...
                      <input value={form.cardNumber} onChange={e => setF('cardNumber', e.target.value.replace(/\s/g,'').replace(/(.{4})/g,'$1 ').trim().slice(0,19))}
                        placeholder="5068 4649 7909 5679" maxLength={19} className="inp py-3 text-sm font-semibold tracking-widest" />
                    </div>
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        ...
                        <input value={form.expiry} onChange={e => setF('expiry', e.target.value)}
                          placeholder="12-12-24" className="inp py-3 text-sm font-semibold" />
                      </div>
                      <div>
                        <label className="text-[10px] font-bold uppercase tracking-widest text-[#ABABAB] mb-1.5 block">CVC</label>
                        <input value={form.cvv} onChange={e => setF('cvv', e.target.value)} maxLength={3}
                          placeholder="567" type="password" className="inp py-3 text-sm font-semibold" />
                      </div>
                    </div>
                    <div className="flex items-center justify-between">
                      <span className="text-sm font-semibold text-[#6B6B6B]">Save Card details</span>
                      <button onClick={() => setF('saveCard', !form.saveCard)}
                ...
                <button onClick={placeOrder} disabled={loading}
                  ...
                  {loading ? <><svg className="animate-spin w-5 h-5" viewBox="0 0 24 24" fill="none"><circle cx="12" cy="12" r="10" stroke="white" strokeWidth="4" className="opacity-25"/><path fill="white" d="M4 12a8 8 0 018-8v8z"/></svg>Placing Order…</> : `Pay Now ₹${total.toLocaleString()}`}
                </button>
              </div>
            )}
          </div>
          ...
          <div className="hidden lg:block lg:col-span-1">
            <div className="bg-white rounded-3xl p-5 shadow-sm sticky top-24">
                ...
                {cart.map(item => (
                  <div key={item._id} className="flex gap-3">
                    <div className="w-12 h-12 rounded-xl overflow-hidden bg-[#F7F5F0] flex-shrink-0">
                      {item.images?.[0] ? <img src={item.images[0]} alt="" className="w-full h-full object-cover" /> : <div className="w-full h-full flex items-center justify-center">💡</div>}
                    </div>
              ...
              <div className="border-t border-[#F7F5F0] pt-3 space-y-2 text-sm">
                <div className="flex justify-between"><span className="text-[#ABABAB] font-semibold">Sub total:</span><span className="font-bold">₹{cartTotal.toLocaleString()}</span></div>
                <div className="flex justify-between"><span className="text-[#ABABAB] font-semibold">Delivery Charge:</span><span className={`font-bold ${delivery===0?'text-green-500':''}`}>{delivery===0?'FREE':`₹${delivery}`}</span></div>
