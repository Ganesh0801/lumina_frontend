...
function StatCard({ title, value, sub, icon: Icon, accent = '#C9A227' }) {
  return (
    <div style={{ ...card, padding: '14px 12px', display: 'flex', alignItems: 'flex-start', gap: 10 }}>
      <div style={{ width: 38, height: 38, borderRadius: 10, flexShrink: 0, background: `${accent}18`, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <Icon size={18} style={{ color: accent }} />
      </div>
      <div style={{ minWidth: 0, flex: 1 }}>
        <p style={{ color: '#8A7A5A', fontSize: 11, fontWeight: 600, marginBottom: 2 }}>{title}</p>
        <p style={{ color: '#E8D8A0', fontSize: 18, fontWeight: 900, lineHeight: 1 }}>{value}</p>
...
export default function AdminDashboard() {
  ...
  return (
    <AdminLayout title="Dashboard">
      ...
      {/* ── KPI CARDS — 2 cols mobile, 4 cols ≥ 1024px ── */}
      <div style={{ display: 'grid', gap: 10, marginBottom: 14 }} className="dash-grid-4">
      ...
      {/* ── SECONDARY STATS — 3 cols, responsive ── */}
      <div style={{ display: 'grid', gap: 10, marginBottom: 14 }} className="dash-grid-3">
        {[
      ...
      {/* ── CHARTS — stacked mobile, side-by-side ≥ 1024px ── */}
      <div style={{ display: 'grid', gap: 12, marginBottom: 14 }} className="dash-grid-2">
        ...
        <div style={{ ...card, padding: '14px 12px' }}>
          ...
          {chartData.length > 0 ? (
            <ResponsiveContainer width="100%" height={170}>
              <AreaChart data={chartData} margin={{ top: 0, right: 5, left: 0, bottom: 0 }}>
                <defs>
                ...
                <YAxis tick={{ fill: '#6B5B30', fontSize: 10 }} axisLine={false} tickLine={false} width={38} />
            ...
            <div style={{ height: 170, display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#6B5B30', fontSize: 13 }}>No data yet</div>
          )}
        </div>
        ...
        <div style={{ ...card, padding: '14px 12px' }}>
          ...
          {pieData.length > 0 ? (
            ...
            <div style={{ height: 120, display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#6B5B30', fontSize: 13 }}>No data yet</div>
          )}
        </div>
      </div>

      {/* ── RECENT ORDERS + LOW STOCK ── */}
      <div style={{ display: 'grid', gap: 12 }} className="dash-grid-2">

        {/* Recent orders */}
        <div style={{ ...card, overflow: 'hidden' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 14px', borderBottom: '1px solid rgba(201,162,39,0.08)' }}>
          ...
          {recentOrders.length === 0 ? (
            ...
            <>
              ...
              <div className="dash-orders-mobile">
                {recentOrders.map(o => (
                  <div key={o._id} style={{ padding: '11px 14px', borderBottom: '1px solid rgba(201,162,39,0.05)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <div>
                      <p style={{ color: '#C9A227', fontSize: 12, fontWeight: 700 }}>{o.orderNumber}</p>
                      <p style={{ color: '#8A7A5A', fontSize: 11, marginTop: 1 }}>{o.user?.name || 'N/A'}</p>
                    </div>
              ...
              {/* Desktop: table */}
              <div className="dash-orders-desktop" style={{ overflowX: 'auto' }}>
                <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 380 }}>
                  <thead>
                    <tr style={{ background: 'rgba(201,162,39,0.04)' }}>
                      {['Order', 'Customer', 'Total', 'Status', 'Date'].map(h => (
                        <th key={h} style={{ padding: '9px 12px', textAlign: 'left', fontSize: 10, fontWeight: 700, color: '#6B5B30', textTransform: 'uppercase', letterSpacing: 1 }}>{h}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                    {recentOrders.map((o, i) => (
                      <tr key={o._id} style={{ borderBottom: '1px solid rgba(201,162,39,0.05)', background: i % 2 === 0 ? 'transparent' : 'rgba(201,162,39,0.02)' }}>
                        <td style={{ padding: '10px 12px', fontSize: 12, color: '#C9A227', fontWeight: 700 }}>{o.orderNumber}</td>
                        <td style={{ padding: '10px 12px', fontSize: 12, color: '#E8D8A0' }}>{o.user?.name || 'N/A'}</td>
                        <td style={{ padding: '10px 12px', fontSize: 12, color: '#C9A227', fontWeight: 700 }}>₹{o.total?.toLocaleString()}</td>
                        <td style={{ padding: '10px 12px' }}><StatusBadge status={o.status} /></td>
                        <td style={{ padding: '10px 12px', fontSize: 11, color: '#6B5B30', whiteSpace: 'nowrap' }}>
        ...
        {/* Low stock */}
        <div style={{ ...card, overflow: 'hidden' }}>
          <div style={{ padding: '12px 14px', borderBottom: '1px solid rgba(201,162,39,0.08)', display: 'flex', alignItems: 'center', gap: 7 }}>
            ...
            {lowStock.length === 0 ? (
              ...
              <div key={p._id} style={{ display: 'flex', alignItems: 'center', gap: 9, padding: '8px 9px', background: 'rgba(248,113,113,0.06)', borderRadius: 9, border: '1px solid rgba(248,113,113,0.10)' }}>
                <div style={{ width: 34, height: 34, borderRadius: 8, overflow: 'hidden', background: '#2A2416', flexShrink: 0 }}>
                  {p.images?.[0]
                    ? <img src={resolveImg(p.images[0])} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} onError={e => { e.target.style.display = 'none'; }} />
      ...
      <style>{`
        ...
        .dash-orders-mobile  { display: flex; flex-direction: column; }
